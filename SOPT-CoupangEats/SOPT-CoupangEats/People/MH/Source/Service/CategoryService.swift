...
struct CategoryService {
    ...
    func subcategoryloading(id: Int, completion: @escaping (NetworkResult<Any>)->Void) {
        let URL = APIConstants.categoryURL + "/\(id)"
        let headers: HTTPHeaders = ["Content-Type" : "application/json"]
        Alamofire.request(URL, method: .get, parameters: nil, encoding: JSONEncoding.default, headers: headers).responseData { response in
            switch response.result {
                ...
                if let value = response.result.value {
                    if let status = response.response?.statusCode {
                        ...
                        case 200:
                            do {
                                let decoder = JSONDecoder()
                                let result = try decoder.decode(ListPro<DataClass3>.self, from: value)
